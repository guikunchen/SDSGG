...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        if self.custom_eval:
            ...
            split_mask, self.gt_boxes, self.gt_classes, self.gt_attributes, self.relationships = load_graphs(
                self.roidb_file, self.split, num_im, num_val_im=5000,
    ...
    def _filter_labels(self, obj_names, prdc_names, split):
        ...
        # begin filtering
        new_gt_boxes, new_gt_classes, new_gt_attributes, new_relationships, new_filenames, new_img_info = [[] for _ in
                                                                                                           range(6)]
        ...
        count1 = 0
        for gt_boxes, gt_classes, gt_attributes, relationships, filenames, img_info in zip(self.gt_boxes,
                                                                                           self.gt_classes,
                                                                                           self.gt_attributes,
                                                                                           self.relationships,
                                                                                           self.filenames,
            ...
            if len(cur_gt_boxes) == 0 or len(cur_relationships) == 0:
                ...
                new_relationships.append(
                    np.array(cur_relationships, dtype=relationships.dtype)), new_gt_attributes.append(
                    gt_attributes[list(cur_obj_idx_map_img)])
                new_filenames.append(filenames), new_img_info.append(img_info)
        print(relation_num_dict)
        print(split)
        # save results
        self.gt_boxes, self.gt_classes, self.gt_attributes, self.relationships, self.filenames, self.img_info = new_gt_boxes, new_gt_classes, new_gt_attributes, new_relationships, new_filenames, new_img_info
    ...
    def _export_valid_pair(self):
        info = {}
        info['idx2prdc'] = {i: self.ind_to_predicates[i] for i in range(len(self.ind_to_predicates))}
        info['idx2obj'] = self.categories
        prdc2valid = {}
        for classes, rels in zip(self.gt_classes, self.relationships):
            for trip in rels:
                sub, obj, rel = info['idx2obj'][classes[trip[0]]], info['idx2obj'][classes[trip[1]]], info['idx2prdc'][
                    trip[2]]
                prdc2valid.setdefault(rel, set())
                prdc2valid[rel].add('{}_{}'.format(sub, obj))
        info['prdc2valid'] = prdc2valid
        import pdb;
        pdb.set_trace()
        np.save('/home/chenguikun/projects/sgg_hbe_pro/info.npy', info, allow_pickle=True)
        print('Done')
